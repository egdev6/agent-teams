...
function initializeMetadataOutput(output: any): void {
  if (!output.mode_default) output.mode_default = 'short+diff';
  if (typeof output.max_bullets !== 'number') output.max_bullets = 7;
  if (!Array.isArray(output.schema)) {
    output.schema = ['Resumen', 'Cambios', 'Verificacion'];
  }
  if (!Array.isArray(output.never_include)) {
    output.never_include = [
      'theory',
      'full_files',
      'duplicate_code',
      'unbounded_lists',
...
function normalize(spec: any): any {
  ...
  initializeMetadataOutput(meta.output);
...
function renderAgentTemplate(templateRaw: string, spec: any): string {
...
export async function runCreate(argv: string[]) {
  ...
  // Render template with agent spec data
  const meta = spec._metadata;
  const rendered = renderAgentTemplate(templateRaw, spec);
