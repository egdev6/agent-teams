...
export const EngramBanner: React.FC<EngramBannerProps> = ({
  ...
  return (
    <div className='flex items-center gap-4 rounded-lg border border-status-warning bg-status-warning/10 px-4 py-3'>
      <Database className='h-5 w-5 shrink-0 text-amber-500' />
      <div className='min-w-0 flex-1'>
        <p className='text-sm font-medium'>{title}</p>
        <p className='text-xs text-muted-foreground'>{description}</p>
      </div>
      <Button size='sm' variant='warning' onClick={onSetup} className='shrink-0'>
        {label}
      </Button>
    </div>
  );
