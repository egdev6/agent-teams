...
import type { AgentScore, AgentSpec, RoutingContext } from './types';
...
export class AgentRouter {
  ...
  private scoreAgent(agent: AgentSpec, context: RoutingContext): AgentScore {
    ...
    const intentRatio =
      context.detectedIntents.length > 0
        ? matchedIntents.length / context.detectedIntents.length
        : 0;

    if (matchedIntents.length > 0) {
      reasons.push(
        `Intents: ${matchedIntents.join(', ')} (${(intentRatio * 100).toFixed(0)}% match)`,
      );
    }

    // Path matching ratio (0 or 1)
    const pathMatch =
      context.currentFile && this.matchesPathGlobs(agent, context.currentFile) ? 1 : 0;
    ...
    if (matchedKeywords.length > 0) {
      reasons.push(
        `Keywords: ${matchedKeywords.join(', ')} (${(keywordRatio * 100).toFixed(0)}% match)`,
      );
    }
