...
import { AgentSpec, DelegationRequest, DelegationResponse } from './types';
...
export class AgentOrchestrator {
  ...
  async delegate(
    ...
    if (handoffDepth >= this.maxHandoffs) {
      this.logger.warn(`Max handoff depth (${this.maxHandoffs}) reached. Stopping delegation chain.`);
      return {
    ...
    if (visitedAgents.has(request.targetAgentId)) {
      this.logger.error(`Loop detected: agent ${request.targetAgentId} already visited in this chain`);
      return {
        agentId: request.targetAgentId,
    ...
    this.logger.info(`Delegating to ${request.targetAgentId} [depth: ${handoffDepth + 1}/${agentMaxHandoffs}]`);
