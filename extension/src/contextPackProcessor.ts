...
export class ContextPackProcessor {
  ...
  private async loadPack(packPath: string, options: ProcessOptions): Promise<string> {
    ...
    if (packPath.startsWith('kit:')) {
      ...
      const kitId = options.kitsPath ? path.basename(options.kitsPath) : '';
      fullPath = path.join(
        options.kitsPath || '',
        'context-packs',
        `${packName}.md`
  ...
  private processLoops(content: string, context: ContextPackContext): string {
    ...
    result = result.replace(eachRegex, (match, collection, block) => {
  ...
  private processVariables(content: string, context: ContextPackContext): string {
    ...
    result = result.replace(techRegex, (match, key) => {
  ...
  private processFilters(content: string): string {
    ...
    result = result.replace(uppercaseRegex, (match, text) => text.trim().toUpperCase());

    // {{lowercase:text}}
    const lowercaseRegex = /\{\{lowercase:([^}]+)\}\}/g;
    result = result.replace(lowercaseRegex, (match, text) => text.trim().toLowerCase());

    // {{capitalize:text}}
    const capitalizeRegex = /\{\{capitalize:([^}]+)\}\}/g;
    result = result.replace(capitalizeRegex, (match, text) => {
