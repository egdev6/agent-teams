...
export class SkillsRegistry {
  ...
  getByRole(role: AgentRole): SkillDefinition[] {
    ...
    return Object.values(this.registry!.skills).filter(
      (skill) =>
        !skill.requires_role || skill.requires_role.includes(role)
    );
  }
    ...
    for (const skillId of skillIds) {
      ...
      if (skill.requires_technologies) {
        ...
        if (missingTechs.length > 0) {
          errors.push(
            `Skill '${skillId}' requires technologies: ${missingTechs.join(', ')}`
          );
        }
      }
      ...
      // Check deprecation
      if (skill.deprecated) {
        const replacement = skill.deprecated_by
          ? ` Use '${skill.deprecated_by}' instead.`
          : '';
      ...
      if (skill.security_level === 'critical') {
        warnings.push(
          `Skill '${skillId}' has critical security level - use with caution`
        );
      } else if (skill.security_level === 'elevated') {
        warnings.push(
          `Skill '${skillId}' has elevated security level - requires careful monitoring`
        );
      }
    }
    ...
    for (const skillId of skillIds) {
      ...
      for (const conflictingSkillId of skill.conflicts_with) {
        if (skillIds.includes(conflictingSkillId)) {
          conflicts.push(
            `'${skillId}' conflicts with '${conflictingSkillId}'`
          );
        }
      }
    }
  ...
  getRecommendations(
    ...
    if (
      technologies.some((t) =>
        ['vitest', 'jest', 'playwright', 'cypress'].includes(t)
      )
    ) {
      recommendations.push({
        skill_id: 'run_tests',
  ...
  private ensureLoaded(): void {
    if (!this.registry) {
      throw new Error(
        'Skills registry not loaded. Call load() first.'
      );
    }
  }
}
