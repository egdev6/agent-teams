...
export class V2Commands {
  ...
  async listTeams(): Promise<void> {
    ...
    try {
      ...
      if (selectedTeam) {
        const teamPath = path.join(workspaceFolder, '.agent-teams', 'teams', `${selectedTeam.label}.yml`);
        const uri = vscode.Uri.file(teamPath);
        const doc = await vscode.workspace.openTextDocument(uri);
        await vscode.window.showTextDocument(doc);
      }
  ...
  async syncTeam(): Promise<void> {
    ...
    try {
      ...
      const dryRun = await vscode.window.showQuickPick([
        { label: 'Apply Changes', description: 'Generate agents to .github/agents/', value: false },
        { label: 'Preview Changes (Dry Run)', description: 'Show what would change without writing files', value: true }
      ], {
      ...
      const result = await vscode.window.withProgress({
        ...
        title: `Syncing team: ${selectedTeam.label}`,
        cancellable: false
      }, async (progress) => {
        progress.report({ increment: 0, message: 'Loading profile...' });

        const syncResult = await this.teamManager.syncTeam(
          workspaceFolder,
          selectedTeam.label,
          { 
            dryRun: dryRun.value,
            showDiff: dryRun.value  // Only show diff in dry-run mode
          }
        );
      ...
      if (dryRun.value) {
        ...
        output.appendLine(`TEAM SYNC PREVIEW: ${selectedTeam.label}`);
        output.appendLine('='.repeat(60));
        output.appendLine('');
