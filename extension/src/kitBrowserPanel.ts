...
export class KitBrowserPanel {
  ...
  public static createOrShow(
    ...
    KitBrowserPanel.currentPanel = new KitBrowserPanel(
      panel,
      extensionUri,
      logger,
      kitsPath
  ...
  private async _createTeamFromKit(kitId: string) {
    try {
      ...
      vscode.window.showInformationMessage(
        `Use the wizard to create a team with kit: ${kitId}`
      );
    } catch (error) {
   ...
   */
  private _getHtmlContent(kits: Array<{ id: string; manifest: KitManifest; path: string }>): string {
    const kitsJson = JSON.stringify(kits.map(k => ({
      id: k.id,
