...
function usage() {
  console.log('Usage: ts-node tools/validate-agent.ts --agents <agents_dir> --schema <schema_path>');
}
...
function findMarkdownFiles(dir: string): string[] {
  ...
  for (const e of entries) {
    ...
    else if (e.isFile() && (e.name.endsWith('.md') || e.name.endsWith('.markdown'))) results.push(full);
  }
  return results;
}
